    ...
    for project_type, cmd, filename in plan:
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                print(f"  [FAIL] {project_type} setup failed (exit {process.returncode})")
                _error(f"command '{cmd}' exited with code {process.returncode}")
...
def _diagnose_and_retry(failed_cmd, output, filename, target_dir):
    ...
    system_tools = {"choco", "brew", "apt-get", "apt", "sudo", "winget", "pacman", "yum", "dnf", "apk"}
